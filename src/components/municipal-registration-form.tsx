...
export function MunicipalRegistrationForm({ organizationId, initialValue }: { organizationId: string; initialValue: string | null }) {
  ...
  return <div className="v2-panel-subsection"><h3>Inscrição municipal</h3><p>Atualize somente com dado confirmado pela empresa.</p><form onSubmit={save} className="company-form"><div className="company-form-grid"><label className="field company-field" htmlFor="municipal-registration"><span>Inscrição municipal</span><input id="municipal-registration" className="input" value={municipalRegistration} onChange={(event) => setMunicipalRegistration(event.target.value)} maxLength={80} required /></label></div><div className="company-form-actions"><button className="button primary" disabled={saving}>{saving ? "Salvando…" : "Salvar inscrição municipal"}</button></div>{message ? <p className={message.includes("salva") ? "alert success" : "alert error"} role="status">{message}</p> : null}</form></div>;
}
