...
export function CertificateManager({organizationId,organizationTaxId,initialCertificate,canWrite}:{organizationId:string;organizationTaxId:string;initialCertificate:Certificate|null;canWrite:boolean}){
  ...
  async function validateCertificate(){setValidatingCertificate(true);setValidationResult(null);setValidationError("");try{const response=await fetch(`/api/admin/organizations/${organizationId}/certificate/preflight`);const result=await response.json();if(!response.ok){setValidationResult("FAIL");setValidationError(result.code??"PREFLIGHT_FAILED");return;}setValidationResult("PASS");}catch{setValidationResult("FAIL");setValidationError("PREFLIGHT_FAILED");}finally{setValidatingCertificate(false);}}
