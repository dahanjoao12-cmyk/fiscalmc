...
export function CancellationReviewActions({ id, status }: { id: string; status: string }) {
  ...
  async function update(action: "review" | "deny") { setLoading(true); setMessage(""); try { const response = await fetch(apiUrl(`/api/admin/cancellation-requests/${id}`), { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) }); const body = await response.json() as { error?: string; message?: string }; setMessage(response.ok ? body.message ?? "Solicitação atualizada." : body.error ?? "Não foi possível atualizar."); if (response.ok) router.refresh(); } catch { setMessage("Não foi possível atualizar."); } finally { setLoading(false); } }
