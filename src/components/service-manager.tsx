...
export function ServiceManager({ organizationId, municipalityCode, initialServices, catalogAvailable, initialSelectedServiceId }: { organizationId: string; municipalityCode: string; initialServices: ManagedService[]; catalogAvailable: boolean; initialSelectedServiceId?: string | null }) {
  ...
  function close() { setOpen(false); setEditingId(null); setSelected(null); setMappings([]); setMappingLoading(false); setNbsCandidates([]); setNbsLoading(false); setForm(blank); setInfoMessage(""); setError(""); }
  function selectCode(code: Code) { setMappings([]); setMappingLoading(true); setNbsCandidates([]); setNbsLoading(true); setSelected(code); setForm((value) => ({ ...value, municipalServiceMappingId: null, name: value.name || code.description.slice(0, 120) })); }
