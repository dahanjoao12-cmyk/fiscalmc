...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="page v2-page invoice-document"><PageHeader backHref="/app/notas" backLabel="Notas" title={title} description={`Competência ${formatDate(invoice.service_date)}`} actions={<StatusBadge tone={presentation.tone}>{presentation.label}</StatusBadge>} />
    ...
    {hasNfseXml ? <section className="v2-panel v2-document-section"><h2>Documentos</h2><div className="v2-document-actions"><a className="button secondary" href={`/api/invoices/${invoice.id}/artifacts/PDF`}>Baixar PDF</a><a className="button secondary" href={`/api/invoices/${invoice.id}/artifacts/NFSE_XML`}>Baixar XML</a></div>{!hasDanfse ? <p className="v2-document-note">PDF gerado a partir dos dados autorizados da NFS-e.</p> : null}</section> : null}
