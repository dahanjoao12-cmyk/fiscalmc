...
import { RequestCancellationButton } from "@/components/request-cancellation-button";
...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: invoice }, { data: attempts }, { data: artifacts }, { data: cancellations }] = await Promise.all([
    ...
    db.from("fiscal_artifacts").select("artifact_type,created_at").eq("invoice_id", id).eq("organization_id", session.organizationId),
    db.from("cancellation_requests").select("id,status").eq("invoice_id", id).eq("organization_id", session.organizationId).order("created_at", { ascending: false }).limit(1),
  ]);
  if (!invoice) notFound();
  ...
  const service=Array.isArray(invoice.service_templates)?invoice.service_templates[0]:invoice.service_templates;
  return <div className="page v2-page invoice-document"><PageHeader backHref="/app/notas" backLabel="Notas" title={title} description={`Competência ${formatDate(invoice.service_date)}`} actions={<StatusBadge tone={presentation.tone}>{presentation.label}</StatusBadge>} />
    ...
    <div className="v2-document-grid"><section className="v2-panel v2-document-section"><h2><UserRound size={19} aria-hidden />Tomador</h2><dl><div><dt>Nome</dt><dd>{customer?.legal_name ?? "—"}</dd></div><div><dt>CPF/CNPJ</dt><dd>{customer?.tax_id ?? "—"}</dd></div></dl></section><section className="v2-panel v2-document-section"><h2><FileText size={19} aria-hidden />Serviço</h2><dl><div><dt>Serviço</dt><dd>{service?.name ?? "—"}</dd></div><div><dt>Valor</dt><dd className="document-total">{formatCurrency(invoice.amount_cents)}</dd></div></dl></section></div>
    ...
    {invoice.status === "ISSUED" ? <section className="v2-panel v2-document-section"><h2>Documentos</h2><div className="v2-document-actions">{hasNfseXml ? <><a className="button secondary" href={`/api/invoices/${invoice.id}/artifacts/PDF`}>Baixar PDF</a><a className="button secondary" href={`/api/invoices/${invoice.id}/artifacts/NFSE_XML`}>Baixar XML</a></> : null}{!hasNfseXml ? <RecoverInvoiceArtifactsButton invoiceId={invoice.id} scope="client" /> : null}</div>{hasNfseXml && !hasDanfse ? <p className="v2-document-note">PDF gerado a partir dos dados autorizados da NFS-e.</p> : null}</section> : null}
    <section className="v2-document-actions">{invoice.status !== "UNKNOWN" ? <Link className="button primary" href="/app/emitir">Emitir outra NFS-e</Link> : null}{invoice.status === "REJECTED" && presentation.action === "USER_ACTION_REQUIRED" ? <Link className="button secondary" href="/app/tomadores">Corrigir dados do tomador</Link> : null}{invoice.status === "ISSUED" && !cancellation ? <RequestCancellationButton invoiceId={invoice.id} /> : null}{cancellation ? <span className="v2-cancellation-status">Cancelamento: {cancellation.status === "REQUESTED" ? "solicitado" : "em análise"}.</span> : null}</section>
  </div>;
}
