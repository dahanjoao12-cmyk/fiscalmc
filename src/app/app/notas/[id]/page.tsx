...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="page v2-page invoice-document"><PageHeader backHref="/app/notas" backLabel="Notas" title={title} description={`Competência ${formatDate(invoice.service_date)}`} actions={<StatusBadge tone={presentation.tone}>{presentation.label}</StatusBadge>} />
    ...
    <section className="v2-document-actions">{hasDanfse ? <a className="button secondary" href={`/api/invoices/${invoice.id}/artifacts/DANFSE_PDF`}>Baixar DANFSe</a> : null}{hasNfseXml ? <a className="button secondary" href={`/api/invoices/${invoice.id}/artifacts/NFSE_XML`}>Baixar XML</a> : null}{invoice.status !== "UNKNOWN" ? <Link className="button primary" href="/app/emitir">Emitir outra NFS-e</Link> : null}{invoice.status === "REJECTED" && presentation.action === "USER_ACTION_REQUIRED" ? <Link className="button secondary" href="/app/tomadores">Corrigir dados do tomador</Link> : null}{invoice.status === "ISSUED" && !cancellation ? <RequestCancellationButton invoiceId={invoice.id} /> : null}{cancellation ? <span className="v2-cancellation-status">Cancelamento: {cancellation.status === "REQUESTED" ? "solicitado" : "em análise"}.</span> : null}</section>
  </div>;
}
