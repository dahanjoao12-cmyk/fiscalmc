...
export default async function DashboardPage() {
  ...
  return <div className="page v2-page client-dashboard"><PageHeader title="Início" description={`Operação fiscal de ${organization?.legal_name ?? "sua empresa"}.`} />
    ...
    <section className="v2-metrics client-metrics"><MetricTile label="Notas emitidas no mês" value={String(rows.filter((item) => item.status === "ISSUED").length)} /><MetricTile label="Em confirmação" value={String(confirming)} tone={confirming ? "warning" : "default"} /><MetricTile label="Pendências" value={String(pending)} tone={pending ? "warning" : "default"} /></section>
