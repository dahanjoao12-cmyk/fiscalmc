...
export default async function CompanyPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{tab?:string}>}){
  ...
  const db=createAdminClient();const canManageCertificate=can(officeSession.role,"certificate:write");
  const[{data:company},{data:services},{data:taxProfile},{count:catalogCount},{count:customerCount},{data:certificate},{count:accessCount}]=await Promise.all([
    ...
    canManageCertificate?db.from('digital_certificates').select('id,subject,issuer,serial,owner_tax_id,valid_from,valid_until,status,created_at').eq('organization_id',id).is('replaced_at',null).maybeSingle():Promise.resolve({data:null}),
