...
export default async function CompanyPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string; service?: string }> }) {
  ...
  return <div className="page v2-page company-detail-page">
    ...
    {tab === "issue" ? <section className="v2-issue-admin"><EmissionPreflight organizationId={company.id} canRun={canRunEmissionPreflight} /><IssueForm customers={issueCustomers} services={issueServices} issuanceOrganizationId={company.id} requiresProductionConfirmation={process.env.NFSE_ENV?.toLowerCase() === "production"} /></section> : null}
