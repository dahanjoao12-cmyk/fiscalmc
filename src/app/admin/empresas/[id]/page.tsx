...
export default async function CompanyPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{tab?:string}>}){
  ...
  const[{data:company},{data:services},{data:taxProfile},{count:catalogCount},{count:customerCount},{count:certificateCount},{count:accessCount}]=await Promise.all([
    ...
    db.from('digital_certificates').select('id',{count:'exact',head:true}).eq('organization_id',id).is('replaced_at',null),
