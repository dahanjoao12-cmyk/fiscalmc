...
const querySchema = z.object({ q: z.string().trim().max(120).optional(), page: z.coerce.number().int().min(0).max(10000).default(0) });
const pageSize = 16;

/** Commercial catalog only: fiscal identifiers and mappings never leave this route. */
export async function GET(request: Request) {
  try {
    const session = await requireSessionOrganization();
    if (session.role !== "CLIENT_USER") throw new Error("FORBIDDEN_CLIENT_CATALOG");
    ...
    const page = data ?? [];
    const existing = page.length ? await db.from("service_templates").select("national_service_code_id").eq("organization_id", session.organizationId).in("national_service_code_id", page.map((item) => item.id)) : { data: [] };
