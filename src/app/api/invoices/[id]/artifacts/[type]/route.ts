...
export async function GET(_request: Request, context: { params: Promise<{ id: string; type: string }> }) {
  try {
    ...
    const { data: invoice } = await createAdminClient().from("invoices").select("id,nfse_number,access_key,service_date,amount_cents,description").eq("id", id).eq("organization_id", session.organizationId).maybeSingle();
    if (!invoice) return NextResponse.json({ error: "Documento não encontrado." }, { status: 404 });
    if (type === "PDF") {
      const official = await downloadAuthorizedFiscalArtifact({ invoiceId: id, organizationId: session.organizationId, artifactType: "DANFSE_PDF", nfseNumber: invoice.nfse_number });
