...
export async function GET(_request: Request, context: { params: Promise<{ id: string; type: string }> }) {
  try {
    ...
    const { data: invoice } = await db.from("invoices").select("id,organization_id,nfse_number,access_key,service_date,amount_cents,description").eq("id", id).maybeSingle();
    ...
    if (type === "PDF") {
      const official = await downloadAuthorizedFiscalArtifact({ invoiceId: id, organizationId: invoice.organization_id, artifactType: "DANFSE_PDF", nfseNumber: invoice.nfse_number });
