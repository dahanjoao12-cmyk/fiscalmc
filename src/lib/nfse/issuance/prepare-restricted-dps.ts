...
import { formatDpsDateTimeSaoPaulo } from "../dps/date-time";
...
export async function prepareRestrictedDps(input:{organizationId:string;document:FiscalDocumentDomain;organization:OrganizationInput;customer:CustomerInput;service:{nationalTaxCode:string;dpsMunicipalTaxCode:string;nbsCode?:string|null;locationMunicipalityCode:string};fiscal:DpsFiscalConfiguration;environment?:NFSeEnvironment;issuerMunicipalRegistrationEmission?:"SEND"|"OMIT";onStage?:(stage:RestrictedDpsPreparationStage)=>void}){
  ...
  const model=mapToDpsModel(input.document,{issuer,customer,serviceLocation:{municipalityCode:input.service.locationMunicipalityCode},dpsMunicipalTaxCode:input.service.dpsMunicipalTaxCode||undefined,nbsCode:input.service.nbsCode??undefined,fiscal:input.fiscal,emittedAt:formatDpsDateTimeSaoPaulo(new Date()),applicationVersion:"FISCALMC_0.1",environment:input.environment==="PRODUCTION"?"1":"2"});
  ...
  const signedValidation=await validateDpsXml(signedXml,input.environment);
