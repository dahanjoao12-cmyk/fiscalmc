...
export class SefinRestrictedReconciliationClient implements UnknownInvoiceLookup{
  ...
  async findByDps(input:{organizationId:string;dpsIdentifier:string}):Promise<OfficialReconciliationResult>{
    ...
    const nfseUrl=`${endpoints.PRODUCTION_RESTRICTED}/nfse/${encodeURIComponent(accessKey)}`;
    const nfseResponse=await this.http.requestText({url:nfseUrl,organizationId:input.organizationId});
    if(nfseResponse.status===404)return{status:"UNKNOWN"};
    ...
    if(!parsedNfse.success||parsedNfse.data.chaveAcesso!==accessKey)throw new SafeFiscalError("INVALID_API_RESPONSE","A resposta de consulta da NFS-e é inconsistente.");
    ...
    return{status:"ISSUED",accessKey,nfseNumber,...(parsedNfse.data.dataHoraProcessamento?{issuedAt:parsedNfse.data.dataHoraProcessamento}:{})};
  }
}
