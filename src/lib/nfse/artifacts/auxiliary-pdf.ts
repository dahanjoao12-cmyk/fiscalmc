...
export async function buildAuthorizedNfseAuxiliaryPdf(input: { xml: Uint8Array; invoice: { nfseNumber?: string | null; accessKey?: string | null; serviceDate: string; amountCents: number; description: string } }) {
  ...
  y -= 8; page.drawRectangle({ x: 48, y, width: 4, height: 16, color: orange }); page.drawText("Prestador", { x: 62, y: y + 3, size: 12, font: bold, color: navy }); y -= 24; line("Nome", first(values, "xNome", "razaoSocial")); line("CNPJ", first(values, "CNPJPrest", "cnpj"));
  y -= 8; page.drawRectangle({ x: 48, y, width: 4, height: 16, color: orange }); page.drawText("Tomador", { x: 62, y: y + 3, size: 12, font: bold, color: navy }); y -= 24; line("Nome", first(values, "xNomeTom", "xNome")); line("CPF/CNPJ", first(values, "CPFTom", "CNPJTom", "cpf", "cnpj"));
